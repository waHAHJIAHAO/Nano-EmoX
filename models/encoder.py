...
from nano_emox.models.ImageBind.models.imagebind_model import ImageBindModel, ModalityType
from nano_emox.models.ImageBind.models import imagebind_model
...
class CLIP_VIT_LARGE(Blip2Base):
    ...
    def forward(self, image, raw_image):
        ...
        embeddings = self.model.get_image_features(inputs) # [(b, t) h] 提取每一帧图片的[cls] 投影到768

        embeddings = einops.rearrange(embeddings, '(b t) h -> b t h', b=batch_size, t=time_length)
        return embeddings


@registry.register_visual_encoder("CLIP_VIT_LARGE_Plus")
class CLIP_VIT_LARGE_Plus(Blip2Base):
    def __init__(self):
        super(CLIP_VIT_LARGE_Plus, self).__init__()

        print('====== Loading CLIP_VIT_LARGE_Plus,for efficient adaptor ======')
        model_dir = config.PATH_TO_VISUAL['CLIP_VIT_LARGE']
        self.model = AutoModel.from_pretrained(model_dir)
        self.processor  = AutoFeatureExtractor.from_pretrained(model_dir)
        ...
        self.hidden_size = self.model.config.vision_config.hidden_size # 1024
        self.dtype = self.model.config.torch_dtype
        print('====== All these parameters are fixed during training!! ======')

    #特征选择，返回多尺度特征和单尺度特征
    def feature_select(self,image_forward_outs,batch_size,time_length,layers=[12,16,22]):
        ...
        if self.select_feature == 'patch':
            ...
            image_features = image_features[:, :1]
            image_features_multi = image_features_multi[:, :1]
            image_features = einops.rearrange(image_features, '(b t) 1 h -> b t h', b=batch_size, t=time_length)
            image_features_multi = einops.rearrange(image_features_multi, '(b t) 1 h -> b t h', b=batch_size, t=time_length)
        else:
            raise ValueError(f'Unexpected select feature: {self.select_feature}')
        return image_features, image_features_multi        

    # image encoding: [b c t h w] => [b t h] 输出多尺度特征
    def forward(self, image, raw_image):
        device = raw_image.device
        batch_size, _, time_length, _, _ = raw_image.size()
        raw_image = einops.rearrange(raw_image, 'b c t h w -> (b t) h w c')
        ...
        image_forward_outs = self.model.vision_model(pixel_values=inputs, output_hidden_states=True)
        
        image_features, image_features_multi = self.feature_select(image_forward_outs, batch_size, time_length) # (b,t,h),(b,t,h*3),h=1024

        return (image_features.to(self.dtype),image_features_multi.to(self.dtype))
...
# 接入videollama3的siglip avt版本，但是没写完
@registry.register_visual_encoder("SigLIP_AVT")
class SigLIP_AVT(Blip2Base):
    def __init__(self):
        super(SigLIP_AVT, self).__init__()

        print('====== Loading SigLIP_AVT ======')
        model_dir = config.PATH_TO_VISUAL['SigLIP_AVT']
        self.model = AutoModel.from_pretrained(model_dir)
        self.processor  = AutoImageProcessor.from_pretrained(model_dir)
...
class HUBERT_LARGE(Blip2Base):
    def __init__(self):
        ...
        # Prevent train mode changes
        def _disabled_train_model(mode=True):
            return self.model
        self.model.train = _disabled_train_model
...
@registry.register_acoustic_encoder("HUBERT_LARGE_Plus")
class HUBERT_LARGE_Plus(Blip2Base):
    def __init__(self):
        super(HUBERT_LARGE_Plus, self).__init__()

        print('====== Loading HUBERT_LARGE_Plus,for efficient adaptor ======')
        model_dir = config.PATH_TO_AUDIO['HUBERT_LARGE']
    ...
    #特征选择，返回多尺度特征和单尺度特征,hubert 24层，hiddensize=1024
    def feature_select(self,acoustic_forward_outs,batch_size,time_length,layers=[16,18,22]):
        acoustic_feature_list = []
        for l in layers:
            acoustic_feature = acoustic_forward_outs[l].mean(dim=1)   # ((b t), T, D) -> ((b t), D)
            acoustic_feature = einops.rearrange(acoustic_feature,'(b t) h -> b t h', b=batch_size, t=time_length) # (b ,t, D)
            acoustic_feature_list.append(acoustic_feature) 
        acoustic_features_multi = torch.cat(acoustic_feature_list, dim=2) # hidden size = 3*1024
        return acoustic_features_multi    # ((b t), T, D) , ((b t), T, D*3)
    ...
    def forward(self, audio, raw_audio):
        ...
        layer_ids = [-4, -3, -2, -1]
        input_values = self.feature_extractor(raw_audio, sampling_rate=16000, return_tensors="pt").input_values # [(b t), s],s=32000
        input_values = input_values.to(device)
        acoustic_forward_outs = self.model(input_values[0], output_hidden_states=True).hidden_states # tuple of ((b t), T, D) # T大小是hubert卷积决定，T=
        feature = torch.stack(acoustic_forward_outs)[layer_ids].mean(dim=0)  # ((b t), T, D) 堆叠后四层的隐藏状态
        feature = feature.mean(dim=1) # ((b t), D) 取最后四层transformer输出，再在时间步上求平均值，获得全局的特征
        acoustic_features = einops.rearrange(feature, '(b t) h -> b t h', b=batch_size, t=time_length)
        
        acoustic_features_multi = self.feature_select(acoustic_forward_outs,batch_size,time_length) # ((b t), T, D*layer)

        return (acoustic_features.to(self.dtype),acoustic_features_multi.to(self.dtype))
