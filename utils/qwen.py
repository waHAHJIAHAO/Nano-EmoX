...
Input: {reason}; Output: """
    ...
    if batch_reasons is not None:
        prompt_list = [func_prompt_template(reason) for reason in batch_reasons]
...
def reason_merge_qwen(model=None, tokenizer=None, llm=None, sampling_params=None,
    ...
    def func_prompt_template(reason, subtitle):
        ...
        if reason != '':
            reason_merge = f"Clue: {reason}；Subtitle: {subtitle}"
...
Output:"
    ...
    if reason is not None:
        ...
        return response
    if batch_reasons is not None:
        prompt_list = [func_prompt_template(r, s) for r, s in zip(batch_reasons, batch_subtitles)]
        response_list = get_completion_qwen_bacth(llm, sampling_params, tokenizer, prompt_list)
        return response_list
...
Input: {reason}; Output: """
    ...
    if batch_reasons is not None:
        prompt_list = [func_prompt_template(reason) for reason in batch_reasons]
        response_list = get_completion_qwen_bacth(llm, sampling_params, tokenizer, prompt_list)
        response_list = [func_onehot_label_polish(r) for r in response_list]
        return response_list
