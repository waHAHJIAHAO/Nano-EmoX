...
class BaseDataset():
    def __init__(self, vis_processor=None, txt_processor=None, img_processor=None, model_cfg=None, dataset_cfg=None,
        ...
        self.num_facial_query_token = model_cfg.num_facial_query_token
        ...
        # 这里token的设置和 nano_emox.py 中的一致 (所以这部分调用改成全局调用了)
    ...
    def get_needed_data(self, face_or_frame):
        if face_or_frame == 'faceframe': # (face, frame, audio, text)
            ...
            needed_data = ['frame']
        elif face_or_frame == 'face_frame': # (frame, face)
            needed_data = ['face', 'frame']
        elif face_or_frame == 'face_frame_text': # (face, frame, text)
            needed_data = ['face', 'frame']
        return needed_data
    ...
    def read_frame_face_audio_text(self, video_path=None, face_npy=None, audio_path=None, image_path=None):
        ...
        sample_data['raw_frames'] = raw_frame
        # print (sample_data)

        # step2: read (raw_face, face)
        face, raw_face = None, None
        # if face_npy is not None and 'face' in self.needed_data: # 由于使用了额外的facial encoder，因此不需要读取facenpy了
    ...
    def func_get_qa_description(self, sample, question_only=False):
        # 定义问题列表
        questions = ["[Inference]Evaluate all available clues—visual, auditory, and physiological. What is the final conclusion about the subject's emotion, and which pieces of evidence were most critical to this determination?",
        ...
        "[Inference]Analyze the complete emotional narrative. Considering the visual context, vocal characteristics, and facial action units, what is the most accurate emotional assessment and why?",
        "[Inference]Based on the synthesis of multimodal data including video, audio, and facial expressions, what is the subject's comprehensive emotional state? Provide a detailed rationale with evidence from each modality.",
        ]
        # 随机选择一个问题
    ...
    def func_get_qa_ovlabel(self, sample, question_only=False):
        questions = ["[Recogn_OV] Please recognize all possible emotional states of the character.",
                    "[Recogn_OV] Can you identify every emotional state the character could be displaying?",
                    "[Recogn_OV] Recognize all the possible emotional states the character might be feeling in this context.",
                    "[Recogn_OV] Identify all possible emotions the character may be experiencing.",
                    "[Recogn_OV] What are the various emotional states the character could potentially be in?",
        ]
    ...
    def func_get_mer_qa_w_think(self, sample, question_only=False):
        question = f"Please recognize emotional states of the character. Output the thinking process in <think></think> and final answer in <answer></answer> tags."
        if question_only:
            return question
        else:
            return {
                'question': question,
                'answer':   f"<think>{sample['description']}</think><answer>{sample['ovlabel']}</answer>."
                }

    def func_get_qa_onehot_w_candidates(self, sample, question_only=False):
        question = f"[Recognition] Please select the label that can best describe the person's emotional state from the provided candidate labels: {self.candidate_labels}."
    ...
    def func_get_qa_onehot_wo_candidates(self, sample, question_only=False):
        question = "[Recognition] Please recognize the character's most likely emotional state."
    ...
    """AvaMerG生成包含COE推理链的共情回应QA对"""
    def func_get_qa_empathic_response_with_coe(self, sample, question_only=False, name=None):
        
        if question_only:
            # 当question_only为True且sample为None时，通过name参数获取上下文信息
            if hasattr(self, 'name2context') and hasattr(self, 'name2currentInput') and name:
                context = self.name2context.get(name, "No Context available.")
                current_user_input = self.name2currentInput.get(name, "No current input available.")
            else:
                context = "No context available."
                current_user_input = "No current input available."
            
            question = f"Context: {context}\n" \
                f"current user input:{current_user_input}\n" \
                f"[Interaction] You are an empathetic listener，your goal is to understand the user’s emotions and intentions, and respond or comfort them with appropriate language that helps them feel understood and cared for. "\
                f"Avoid rushing into your response; instead, carefully engage in a step-by-step, in-depth analysis before providing an answer."\
                f"Please analyze using Chain of Empathy (Firstly, Event scenario:Reflect on the event scenarios that arise from the ongoing dialogue" \
                f"Secondly, User's emotion:Analyze both the implicit and explicit emotions conveyed by the user,"\
                f"Thirdly, the emotion cause:Infer the underlying reasons for the user's emotions."\
                f"Fourthly, the goal to response:Determine the goal of your response in this particular instance, such as alleviating anxiety, offering reassurance, or expressing understanding.)in <think></think> tags and with Line break," \
                f" then provide your empathetic response:"

            return question

        current_user_input = sample['current_user_input']
        ...
        question = f"Context: {context}\n" \
                ...
                f"Avoid rushing into your response; instead, carefully engage in a step-by-step, in-depth analysis before providing an answer."\
                f"Please analyze using Chain of Empathy (Firstly, Event scenario:Reflect on the event scenarios that arise from the ongoing dialogue" \
                f"Secondly, User’s emotion:please analyze User’s emotion mostly like emotions conveyed by the user(speaker)," \
                f"Thirdly, the emotion cause:Infer the underlying reasons for the user’s emotions."\
                f"Fourthly, the goal to response:Determine the goal of your response in this particular instance, such as alleviating anxiety, offering reassurance, or expressing understanding.)in <think></think> tags and with Line break,then provide your empathic response:"
            # 构建包含COE推理链的完整回答 
            
        coe_analysis = f"Firstly, the event scenario of this conversation is: {sample['event_scenario']}\n" \
                        f"Secondly, the emotion of the speaker is {sample['speaker_emotion']}\n" \
                        f"Thirdly, the emotion cause is {sample['emotion_cause']}\n" \
                        f"Fourthly, the goal to response is {sample['goal_to_response']}"
            
        answer = f"<think>{coe_analysis}</think>\n {sample['response']}"
    ...
    """AvaMerG生成speaker情绪判别QA对"""
    def func_get_qa_emotion_recognition(self,sample,question_only=False):
        question = f"[Recognition] Please select the label that can best describe the speaker's emotional state from the provided candidate labels: {self.candidate_labels}."
        if question_only:
            return question
        else:
            question = f"Context: {sample['context']}\n" \
                    f"current speaker input:{sample['current_user_input']}\n" \
                    f"Base on all input information,please analyze User’s emotion mostly like emotions conveyed by the user(speaker)"
            answer = f"The emotion of the speaker is {sample['speaker_emotion']}." 
    ...
    """训练MELD 和 IEMOCAP数据的情感线索描述"""
    def func_get_emo_clue(self, sample, question_only=False):
        """
        情感线索描述多模态QA任务
        """
        questions = ["[Clue]Based on all the above content, conduct a detailed description of its facial expressions, visual features, and audio emotional clues.",
        "[Clue]Combining all the previous information, make a specific portrayal of its facial expressions, visual elements, and audio emotional signals.",
        "[Clue]According to the complete content above, provide a detailed explanation of its facial state, visual presentation, and audio emotional information.",
        "[Clue]With reference to all the aforementioned information, carry out an accurate description of its facial performance, visual details, and audio emotional clues.",
        ]
        question = random.choice(questions)
        if question_only:
            return question
        else:
            sentiment_analyse = sample.get('emotion_clue_analysis', '')
            return {
                'question': question,
                'answer': sentiment_analyse
            }

    """训练MELD 和 IEMOCAP数据的情感推理任务"""   
    def func_get_description(self, sample, question_only=False):
        """
        生成MELD情绪识别的问答对
        """
        questions = ["[Inference]Evaluate all available clues—visual, auditory, and physiological. What is the final conclusion about the subject's emotion, and which pieces of evidence were most critical to this determination?",
        ...
        if question_only:
            ...
            emotion_reasoning = sample.get('comprehensive_emotion_reasoning', '')
            return {
                'question': question,
                'answer': emotion_reasoning
            }

    def func_get_intention(self, sample, question_only=False):
        """
        生成Intention quetions
        """
        question = f"[Intent]recognize speaker's intention from the provided candidate labels: {self.candidate_labels}."
        if question_only:
            return question
        else:
            intention = sample.get('intent', '')
            return {
                'question': question,
                'answer': intention
            }
    # Open-R1-psy心理健康支持QA
    def func_get_counsel_response_w_think(self, sample, question_only=False):
        prompt_pools=["[Therapy]Play an empathetic mental health therapist: listen non-judgmentally, guide emotional healing.",
        "[Therapy]Play a compassionate wellness coach: foster safe dialogue, support mental well-being.",
        "[Therapy]Play a non-judgmental mental health guide: validate feelings, build emotional resilience.",
        "[Therapy]Play a caring emotional therapist: nurture self-awareness, promote healing journeys.",
        "[Therapy]Play an empathetic mental health supporter: process emotions, guide healthy coping."
        ]
        role= random.choice(prompt_pools)
        question = f"{role} Before responding to the user, first analyze the situation by combining international standard treatment methods such as CBT, ACT, and humanitarian approaches. Wrap the analyzed content within <think></think>. Then, proceed with the response, and wrap the response content within <answer></answer>."
        if question_only:
            return question
        else:
            answer = f"<think>{sample['counselor_think']}</think>\n<answer>{sample['counselor_content']}</answer>"
            return {
                'question': question + f"Chat history is{sample['chat_history']}",
                'answer': answer
            }

    def func_get_counsel_response_wo_think(self, sample, question_only=False):
        prompt_pools=["[Therapy]Play an empathetic mental health therapist: listen non-judgmentally, guide emotional healing.",
        "[Therapy]Play a compassionate wellness coach: foster safe dialogue, support mental well-being.",
        "[Therapy]Play a non-judgmental mental health guide: validate feelings, build emotional resilience.",
        "[Therapy]Play a caring emotional therapist: nurture self-awareness, promote healing journeys.",
        ...
        role= random.choice(prompt_pools)
        question = f"{role} Please directly to the user."
        if question_only:
            return question
        else:
            answer = f"{sample['counselor_content']}"
            return {
                'question': question,
                'answer': answer
            }
    ...
    def get_qa_pairs(self, dataset, label_type, sample):
        ...
        if dataset in ['EMERCoarse', 'EMERFine']:
            ...
            candidates = {
                'description': self.func_get_qa_description(sample),
                'ovlabel':     self.func_get_qa_ovlabel(sample),
                'mer_w_think': self.func_get_mer_qa_w_think(sample),
            }

        elif dataset in ['MER2025OV']:
            candidates = {
                'mer_w_think': self.func_get_mer_qa_w_think(sample),
            }
        ...
        ## case4: instruction dataset
            ...
            candidates = {
                'empathic_response_with_coe' : self.func_get_qa_empathic_response_with_coe(sample),
                'emotion_recognition': self.func_get_qa_emotion_recognition(sample),
            }
        elif dataset in ['MELD_train','IEMOCAP_train']:
            candidates = {
                'emotion_clue_analysis': self.func_get_emo_clue(sample),
                'comprehensive_emotion_reasoning': self.func_get_description(sample),
                'emotion_recognition': self.func_get_qa_onehot_wo_candidates(sample),
            }

        elif dataset in ['CAER_FERV39K','CREMA']:
            candidates = {'emotion_recognition': self.func_get_qa_onehot_w_candidates(sample)}
        
        elif dataset in ['M3ED']:
            candidates = {'emotion_recognition': self.func_get_qa_onehot_wo_candidates(sample)}
        
        elif dataset in ['MIntRec', 'MIntRec2']:
            candidates = {'intent_recognition': self.func_get_intention(sample)}

        elif dataset in ['OpenR1Psy']:
            candidates = {'counsel_response_w_think': self.func_get_counsel_response_w_think(sample),
                          'counsel_response_wo_think': self.func_get_counsel_response_wo_think(sample)}
        else:
            raise ValueError(f"Unknown dataset: {dataset}. Please add support for this dataset in get_qa_pairs method.")

        return candidates[label_type] # 包含 question, answer 两部分内容
    
    ## 整合QApiars、模态信息、字幕信息,构造多模态的提示词
    def get_prompt_for_multimodal(self, face_or_frame, subtitle, user_message):
        ...
        if face_or_frame == 'faceframe': # (face, frame, audio, text)
            ...
            prompt = f"###Human: The audio content is as follows: <Audio><AudioHere></Audio>. " \
                    ...
                    + f"Additionally, We uniformly extract faces information from video: <Video><FaceHere></Video>. "  \
            ...
            prompt = f"###Human: The image content is as follows: <Image><ImageHere></Image>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message} ###Assistant: "
        elif face_or_frame == 'face_frame': # face and frame
            prompt = f"###Human: We uniformly sample raw frames from the video: <Video><FrameHere></Video>. " \
                    + f"Meanwhile, We uniformly extract facial informations from video: <Video><FaceHere></Video>. "  \
                    + f"Now, please answer my question based on all the provided information. {user_message} ###Assistant: "
        elif face_or_frame == 'face_frame_text': # (face, frame, text)
            ...
            prompt =  f"We uniformly sample raw frames from the video: <Video><FrameHere></Video>. "  \
                    + f"We uniformly extract facial informations from video: <Video><FaceHere></Video>. "  \
                    + f"The subtitle of this video is: <Subtitle>{subtitle}</Subtitle>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message} ###Assistant: "
        ...
        ## 这部分是为了和其他 MLLM 进行公平比较，所进行的 ablation study 部分
            ...
            assert subtitle is not None
            prompt =  f"We uniformly extract facial informations from video: <Video><FaceHere></Video>. "  \
                    + f"The subtitle of this video is: <Subtitle>{subtitle}</Subtitle>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message} ###Assistant: "
        elif face_or_frame == 'frame_text': # (frame, text)
            assert subtitle is not None
    ...
    def replace_token_for_multimodal(self, prompt):
        ...
        replace_token = config.DEFAULT_FACE_PATCH_TOKEN * self.num_facial_query_token
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                # step2: read (question, answer) # => 如果 sample 中缺少 qa 对应内容的信息，结果是会报错的
        ...
        return {
            ...
            "raw_frames": sample_data['raw_frames'], # [c=3, frame=8, 224, 224]
    ...
    def collater(self, instances):
        ...
        for sample_type in ['face', 'raw_face', 'frame', 'raw_frames', 'audio', 'raw_audio', 'image', 'raw_image']:
            # 对于已经是复数形式的键名，不再添加's'
            if sample_type.endswith('s'):
                batch_type = sample_type
            else:
        ...
        if 'name' in instances[0]:
            batch['name'] = [inst.get('name', '') for inst in instances]
        if 'ovlabel' in instances[0]:
            batch['ovlabel'] = [inst.get('ovlabel', '') for inst in instances]
